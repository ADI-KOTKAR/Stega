...
def image_encode_result():
    if request.method == 'POST':
      ...
      if file:
        ...
        encryption = True
        encrypt(os.path.join(current_app.config['UPLOAD_IMAGE_FOLDER'], filename))
      else:
        encryption = False
      result = request.form
...
def encrypt(image_1): 
    ...
    adjusted_sample_image = cv2.imread(os.path.join(current_app.config['UPLOAD_IMAGE_FOLDER'], "sample.jpg")).copy()
    adjusted_sample_image = cv2.resize(adjusted_sample_image, (dimensions[1], dimensions[0]))
    cv2.imwrite(os.path.join(current_app.config['UPLOAD_IMAGE_FOLDER'], "adjusted_sample.jpg"), adjusted_sample_image) 
    img1 = cv2.imread(os.path.join(current_app.config['UPLOAD_IMAGE_FOLDER'], "adjusted_sample.jpg")) 
    ...
    cv2.imwrite(os.path.join(current_app.config['UPLOAD_IMAGE_FOLDER'], "encrypted_image.png"), img1) 
...
def decrypt(image_1): 
    ...
    # the encrypted image 
    cv2.imwrite(os.path.join(current_app.config['UPLOAD_IMAGE_FOLDER'], "decrypted_sample.png"), img1) 
    cv2.imwrite(os.path.join(current_app.config['UPLOAD_IMAGE_FOLDER'], "decrypted_secret.png"), img2) 
